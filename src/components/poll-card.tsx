				...
				<div className="space-y-4">
					{poll.options.map((option, _index) => {
						...
						const isWinning =
							...
							percentage ===
								Math.max(
									...poll.options.map((o) => getVotePercentage(o.votes)),
								);
								...
								>
									<div className="flex w-full items-center justify-between">
										<div className="flex items-center gap-3">
											{isSelected && (
												<CheckCircle2 className="h-5 w-5 text-inherit" />
											)}
											<span
												className={`text-left font-medium${isSelected ? " text-inherit" : ""}`}
											>
												{option.text}
											</span>
										</div>
										{showResults && (
											<div className="flex items-center gap-3">
												<div className="text-right">
													<span
														className={`font-bold text-sm${isSelected ? " text-inherit" : ""}`}
													>
														{percentage}%
													</span>
													<div
